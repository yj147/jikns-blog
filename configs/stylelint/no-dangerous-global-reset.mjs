...
const messages = stylelint.utils.ruleMessages(ruleName, {
  reset: (selector, prop) =>
    `不要在全局选择器 "${selector}" 中重置 ${prop}，请在组件级样式里处理。`,
  apply: (selector, value) =>
    `不要在全局选择器 "${selector}" 中 @apply ${value} 来移除 ring/outline。`,
})
...
const dangerousApplyPattern =
  /(ring-0|ring-transparent|outline-none|focus-visible:outline-none|focus:outline-none|border-0)/
...
const ruleFunction = (primaryOption, secondaryOptions = {}) => {
  ...
  const allowSelectors = (secondaryOptions.allowSelectors || []).map(
