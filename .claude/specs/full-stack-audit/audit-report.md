...
**审计日期**: 2025-12-04 **审计范围**: 全栈（前端 + 后端 + 数据层）
**审计维度**: 安全 + 代码质量 + 性能
...
- 自研 JWT 实现默认使用公开的硬编码密钥 `"access-secret-key"` /
  `"refresh-secret-key"`
- 签名算法使用 `sha256(secret || data)` 而非标准 HMAC
- 未校验 iss/aud/alg 字段

**影响**: 若环境变量缺失（常见），攻击者可伪造任意访问/刷新令牌直接通过
`withApiSecurity` 验证
...
**问题**: 评论 API 查询时选择了 `email`、`role`
并返回给前端，前端在无昵称时直接展示邮箱
...
- 断线时 `startPolling` 把 `isPollingFallback` 设为 true，但若未传 `pollFetcher`
  直接返回
...
**问题**: `refreshSecurityState` 直接将 `sessionValid` / `csrfTokenValid`
设为 true，TODO 未实现实际校验
...
**问题**: 将 `access_token` / `refresh_token`
直接放入 JSON 响应返回给前端，未加 HttpOnly/SameSite 保护
...
**问题**: `validateRequestOrigin` 允许默认 `http://localhost:3000` 和硬编码
...
**问题**: `isSaving` / `lastSavedAt`
存在 ref 中，组件不会因保存状态变化而重新渲染
...
**问题**: 两处实现冲突，`fetchAuthenticatedUser`
触发的自愈路径会把管理员自动降级
...
**问题**: 手动清理的 Cookie 名称（`supabase-auth-token`
等）与 Supabase 实际名称不符
