...
- **ID**: task-1
- **Description**: 新增 Playwright
  E2E 场景，验证用户在设置页更新个人资料（昵称、简介、社交链接）后，资料页即时展示一致的数据，覆盖正常更新和网络延迟场景
- **File Scope**:
...
- **ID**: task-2
- **Description**: 为 `lib/profile/stats.getQuickStats` 新增单元测试，为 Profile
  Tabs (posts/activities) 新增集成测试，覆盖空态、分页、错误重试场景
- **File Scope**:
...
- **Dependencies**: None
- **Test Command**:
  `pnpm test -- tests/unit/profile-stats.test.ts tests/integration/profile-tabs.test.tsx --coverage --coverage-reporter=text`
- **Test Focus**:
...
- **ID**: task-3
- **Description**: 扩充
  `tests/integration/notification-center.test.ts`，新增防御性测试用例，覆盖非法参数（非法 type、超限 limit、空 ids）、越权标记、并发标记等场景
- **File Scope**:
  - `tests/integration/notification-center.test.ts` (扩充现有)
  - `app/api/notifications/route.ts` (验证错误处理)
- **Dependencies**: None
- **Test Command**:
  `pnpm test -- tests/integration/notification-center.test.ts --coverage --coverage-reporter=text`
- **Test Focus**:
...
- **ID**: task-4
- **Description**: 扩充 `tests/integration/avatar-upload.test.ts` 和
  `tests/integration/social-links.test.ts`，新增上传失败、Supabase
  metadata 同步失败、管理员代他人更新等异常路径测试
- **File Scope**:
...
- **Dependencies**: None
- **Test Command**:
  `pnpm test -- tests/integration/avatar-upload.test.ts tests/integration/social-links.test.ts --coverage --coverage-reporter=text`
- **Test Focus**:
...
- **ID**: task-5
- **Description**: 新增/扩充
  `tests/e2e/notifications.spec.ts`，验证通知列表的过滤器切换（all/unread）、无限滚动、网络失败恢复等 E2E 场景
- **File Scope**:
