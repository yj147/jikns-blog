...
const improvement = {
  ...
  styleLayoutPercent: ((fontMedians.styleLayout - cssMedians.styleLayout) / fontMedians.styleLayout * 100).toFixed(1),
...
console.log('=== 关键指标对比 ===');
console.log(`Style & Layout: ${fontMedians.styleLayout}ms → ${cssMedians.styleLayout}ms (${improvement.styleLayoutPercent > 0 ? '-' : '+'}${Math.abs(improvement.styleLayoutPercent)}%)`);
console.log(`TTI: ${fontMedians.tti}ms → ${cssMedians.tti}ms (${improvement.ttiPercent > 0 ? '-' : '+'}${Math.abs(improvement.ttiPercent)}%)`);
console.log(`LCP: ${fontMedians.lcp}ms → ${cssMedians.lcp}ms (${improvement.lcpPercent > 0 ? '-' : '+'}${Math.abs(improvement.lcpPercent)}%)`);
console.log(`FCP: ${fontMedians.fcp}ms → ${cssMedians.fcp}ms (${improvement.fcpPercent > 0 ? '-' : '+'}${Math.abs(improvement.fcpPercent)}%)`);
console.log(`Performance Score: ${fontMedians.score} → ${cssMedians.score} (${improvement.score > 0 ? '+' : ''}${improvement.score})`);
