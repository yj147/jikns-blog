...
- 使用 PostgreSQL GIN 索引 +
  `websearch_to_tsquery('simple')`，与 `search_vector` 的 `simple` 字典保持一致，确保 nodejieba 拆分的 token 能命中。
...
2. **搜索无响应**：浏览器禁止 `localStorage`
   时历史记录会禁用，但搜索仍可用；若收到 200 + `QUERY_TOO_SHORT` 或 400 + `QUERY_TOO_LONG`，请调整关键字长度。
