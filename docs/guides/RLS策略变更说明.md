...
- 新建策略：
  - `users_select_public_whitelist`（TO anon）：只读白名单列
    `id/name/avatarUrl/bio/createdAt`。
  - `users_select_self_or_admin`（TO
    authenticated）：自身或管理员可读完整记录，管理员判定通过
    `public.is_admin_user()`。
  - `users_select_service_role`：保持 service_role 完量访问（Prisma 依赖）。
- 列权限改为白名单：仅 anon 拥有上述 5 个列的 SELECT 权限，authenticated 仍保留全列访问。
- 新增 `public.is_admin_user(text)` 安全函数（SECURITY
  DEFINER），避免策略自引用递归。
...
- 由于列权限改为白名单，匿名或携带 anon
  JWT 的请求访问敏感列将返回 403/permission denied。
