...
- **位置**: `lib/repos/search/{posts,activities,users,tags}.ts`
- **机制**:
...
1.  **SQL 迁移脚本**: 修正了 `docs/10-search/搜索功能设计文档.md`
    中的 SQL 示例，明确了 `search_vector` 来源于 `*Tokens`
    分词字段而非原始文本字段。
2.  **服务分层描述**: 更新了文档以反映 Server Actions +
    Repositories 的代码结构，强调了 Prisma Extension 在分词中的核心作用。
3.  **限流策略**: 确认了文档中关于“可配置限流”的描述准确对应
    `lib/rate-limit/search-limits.ts` 的实现。
