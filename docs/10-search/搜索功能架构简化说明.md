...
**修改前**：REST API 和 Server
Actions 可能各自维护一套搜索逻辑，或者依赖厚重的 Service 层 (`lib/services/search.ts`)。

**修改后**：Server Actions 和 REST API (`app/api/search/route.ts`) 均直接调用
`lib/repos/search/unified-search.ts` 或具体的仓储函数。

- ✅ **Server Actions**: `lib/actions/search/search-content.ts` ->
  `lib/repos/search/*`
- ✅ **REST API**: `app/api/search/route.ts` ->
  `lib/repos/search/unified-search.ts`

这种设计确保了无论通过何种方式调用搜索（页面交互或 API 调用），底层执行的查询逻辑、排序规则和降级策略完全一致。
