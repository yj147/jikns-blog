...
- **Server
  Actions 层**：`lib/actions/search/`，UI 组件的主要数据源（`search-content.ts`），负责参数验证、权限检查、限流与结果聚合。
- **REST API 层**：`app/api/search/route.ts`，外部接口或遗留调用入口，直接调用
  `lib/repos/search/unified-search.ts`，与 Server Actions 共享仓储层逻辑。
- **仓储层**：
  - `lib/repos/search/`：包含 `unified-search.ts` (聚合入口) 及 `posts.ts`,
    `activities.ts`, `users.ts`, `tags.ts`。
  - 负责构建具体的 SQL 查询（`search_vector` 全文搜索或 `ILIKE` 降级查询）。
- **数据层增强**：
  - `lib/prisma.ts`：通过 Prisma Client Extension (`searchTokenExtension`) 拦截
    `create`/`update` 操作。
  - **分词机制**：调用 `nodejieba` 将中文内容分词并存入 `*Tokens` 字段（如
    `titleTokens`），供数据库层生成 `tsvector`。
- **UI 层**：
...
- **页面层**：`app/search/page.tsx`，搜索结果页面。
...
-- 注意：这里使用 titleTokens 等分词字段，这些字段由应用层（lib/prisma.ts + nodejieba）自动填充
ALTER TABLE posts
...
| 内容类型 | 搜索字段 (Tokens)    | 索引类型     | 权重 |
| -------- | -------------------- | ------------ | ---- |
| Post     | titleTokens          | tsvector (A) | 最高 |
| Post     | excerpt/seo...Tokens | tsvector (B) | 高   |
| Post     | contentTokens        | tsvector (C) | 中   |
| Activity | contentTokens        | tsvector (A) | 最高 |
| User     | name, bio            | pg_trgm      | N/A  |
| Tag      | name, description    | pg_trgm      | N/A  |

> **注意**: User 和 Tag 目前使用 `pg_trgm` 进行模糊匹配，但 User 模型也具备
> `search_vector` 字段（基于 `nameTokens`, `bioTokens`），未来可升级为全文搜索。
