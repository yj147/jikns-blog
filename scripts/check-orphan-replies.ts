...
async function checkOrphanReplies() {
  ...
  allComments.forEach((c, idx) => {
    console.log(`${idx + 1}. ID: ${c.id.slice(0, 8)}... | parentId: ${c.parentId ? c.parentId.slice(0, 8) + '...' : 'null'} | content: ${c.content.slice(0, 20)}`)
  })
  ...
  orphans.forEach((c, idx) => {
    console.log(`${idx + 1}. ID: ${c.id.slice(0, 8)}... | parentId: ${c.parentId!.slice(0, 8)}... | content: ${c.content.slice(0, 20)}`)
  })
