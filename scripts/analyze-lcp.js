...
if (
  renderBlocking.details &&
  renderBlocking.details.items &&
  renderBlocking.details.items.length > 0
) {
...
if (networkRequests.details && networkRequests.details.items) {
  ...
  criticalResources.forEach((req, i) => {
    ...
    console.log(
      `   时序: ${req.startTime.toFixed(0)}ms - ${req.endTime.toFixed(0)}ms (耗时 ${duration.toFixed(0)}ms)`
    )
  })
}
