...
vi.mock("@/lib/prisma", async () => {
  const { mockManager } = await import("./__mocks__/unified-mock-manager")
  return {
    prisma: mockManager.getPrismaMock(),
  }
})

vi.mock("@/lib/supabase", async () => {
  const { mockManager } = await import("./__mocks__/unified-mock-manager")
  const supabaseMock = mockManager.getSupabaseMock()

  return {
    createClient: vi.fn(() => supabaseMock),
    createServerSupabaseClient: vi.fn(async () => supabaseMock),
    createRouteHandlerClient: vi.fn(async () => supabaseMock),
    createClientSupabaseClient: vi.fn(() => supabaseMock),
    // 直接导出supabase实例用于某些测试
...
const mockFetch = vi.fn((url: string | URL | Request, init?: RequestInit) => {
  ...
  // 特定API的Mock响应
  if (urlString.includes("/api/user/profile") || urlString.includes("/api/user")) {
