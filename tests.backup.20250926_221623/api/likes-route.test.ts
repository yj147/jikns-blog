...
describe("点赞 API 路由测试", () => {
  beforeEach(() => {
    ...
    vi.mocked(routeGuard.getUserOrNull).mockResolvedValue(null)
  })
  ...
  describe("GET /api/likes - 获取点赞状态", () => {
    ...
    it("应该返回点赞状态（登录用户）", async () => {
      ...
      vi.mocked(routeGuard.getUserOrNull).mockResolvedValue(mockUser as any)
      vi.mocked(interactionsLib.getLikeStatus).mockResolvedValue(mockStatus)
    ...
    it("应该返回错误：缺少 targetType", async () => {
      ...
      expect(data.error.message).toContain("Invalid targetType")
    })
    ...
    it("应该返回错误：缺少 targetId", async () => {
      ...
      expect(data.success).toBe(false)
      expect(data.error.code).toBe("VALIDATION_ERROR")
      expect(data.error.message).toContain("targetId is required")
    })
  })
  ...
  describe("POST /api/likes - 切换点赞状态", () => {
    it("应该成功切换点赞状态（已登录用户）", async () => {
      ...
      vi.mocked(routeGuard.requireAuth).mockResolvedValue(mockUser as any)
      ...
      expect(data.success).toBe(true)
      expect(data.data).toEqual(mockStatus)
      expect(interactionsLib.toggleLike).toHaveBeenCalledWith("post", "123", "user-1")
      expect(auditLog.auditLogger.logEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          action: "LIKE_TOGGLE",
          resource: "post:123",
    ...
    it("应该返回错误：未登录", async () => {
      ...
      vi.mocked(routeGuard.requireAuth).mockResolvedValue(
        NextResponse.json(
    ...
    it("应该返回错误：目标不存在", async () => {
      ...
      vi.mocked(routeGuard.requireAuth).mockResolvedValue(mockUser as any)
      vi.mocked(interactionsLib.toggleLike).mockRejectedValue(new Error("post not found"))
    ...
    it("应该成功取消点赞", async () => {
      ...
      expect(data.success).toBe(true)
      expect(data.data).toEqual(mockStatus)
      expect(interactionsLib.toggleLike).toHaveBeenCalledWith("activity", "456", "user-1")
      expect(auditLog.auditLogger.logEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          action: "LIKE_TOGGLE",
          resource: "activity:456",
