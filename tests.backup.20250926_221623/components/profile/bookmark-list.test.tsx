  ...
  describe("加载更多功能", () => {
    it("应该正确加载更多收藏并拼接到列表", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        `/api/bookmarks?action=list&userId=${mockUserId}&cursor=cursor-2&limit=20`
      )
    ...
    it("应该正确传递 cursor 参数", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining("cursor=initial-cursor"))
    })
