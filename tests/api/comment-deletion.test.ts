...
// Mock Prisma（统一导出 default 与 prisma，并使用 mockPrisma 作为 $transaction 上下文）
vi.mock("@/lib/prisma", () => {
  const mockPrisma = {
    comment: {
      findUnique: vi.fn(),
...
describe("Comment Deletion API Tests", () => {
  ...
  describe("Error Handling", () => {
    it("should return 404 when comment not found", async () => {
      vi.mocked(getCurrentUser).mockResolvedValue(mockUser)
      vi.mocked(prisma.comment.findUnique).mockResolvedValue(null as any)
