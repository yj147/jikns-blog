...
describe("动态评论功能测试", () => {
  const renderWithAuth = (ui: ReactNode, user: any = null) => {
  ...
  const mockCommentsHook = (overrides: Record<string, any> = {}) => {
  ...
  describe("评论表单交互", () => {
    it("应该在未登录时显示登录提示", () => {
      renderWithAuth(<CommentForm targetType="activity" targetId="activity-1" />)
    ...
    it("应该处理评论提交", async () => {
      ...
      renderWithAuth(
        <CommentForm targetType="activity" targetId="activity-1" onSuccess={onSuccess} />,
        { id: "user-1", name: "当前用户" }
      )
  ...
  describe("评论展开/收起功能", () => {
    it("应该支持展开和收起回复", async () => {
      ...
      global.fetch = vi.fn().mockResolvedValue({
        ...
        json: async () => ({ success: true, data: mockComments[0].replies ?? [] }),
      } as any)

      renderWithAuth(<CommentList targetType="activity" targetId="activity-1" />, {
        id: "user-1",
        name: "当前用户",
      })
      ...
      // 点击展开回复
      const expandButton = await screen.findByText(/展开 1 条回复/)
  ...
  describe("错误处理", () => {
    it("应该处理加载失败的情况", () => {
      ...
      expect(screen.getByText("加载评论失败，请稍后重试。")).toBeInTheDocument()
    })
