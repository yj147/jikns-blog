...
import {
  ...
  getBookmarkStatus,
  getUserBookmarks,
} from "@/lib/interactions/bookmarks"
...
describe("Bookmarks Service", () => {
  ...
  describe("toggleBookmark", () => {
    ...
    it("应该在文章不存在时抛出错误", async () => {
      ...
      await expect(toggleBookmark(postId, userId)).rejects.toBeInstanceOf(
    ...
    it("应该在文章创建前被删除时抛出 InteractionTargetNotFoundError", async () => {
      vi.mocked(prisma.bookmark.findUnique).mockResolvedValueOnce(null)
      vi.mocked(prisma.post.findFirst).mockResolvedValueOnce({
        id: postId,
        published: true,
      ...
      vi.mocked(prisma.bookmark.create).mockRejectedValueOnce(foreignKeyError)
  ...
  describe("getUserBookmarks", () => {
    ...
    it("应该在同一时间戳下保持稳定的分页游标", async () => {
      const sharedCreatedAt = new Date("2024-02-01T12:00:00.000Z")
      ...
      const result = await getUserBookmarks(userId, { limit: 2 })

      expect(result.items.map((item) => item.id)).toEqual(["bookmark-3", "bookmark-2"])
      expect(result.hasMore).toBe(true)
  ...
  describe("setBookmark（幂等核心方法）", () => {
    const postId = "post-123"
    const userId = "user-456"

    it("setBookmark(desired=true) 应该创建收藏", async () => {
      // Mock 文章存在且已发布
      vi.mocked(prisma.post.findFirst).mockResolvedValueOnce({
        id: postId,
        published: true,
      ...
      // Mock 收藏计数
      vi.mocked(prisma.bookmark.count).mockResolvedValueOnce(1)

      const result = await setBookmark(postId, userId, true)
    ...
    it("setBookmark(desired=false) 应该删除收藏（使用 deleteMany）", async () => {
      // Mock deleteMany（天然幂等）
      vi.mocked(prisma.bookmark.deleteMany).mockResolvedValueOnce({ count: 1 })

      // Mock 收藏计数
      vi.mocked(prisma.bookmark.count).mockResolvedValueOnce(0)

      const result = await setBookmark(postId, userId, false)
    ...
    it("setBookmark(desired=true) 应该处理 P2002 唯一约束冲突（幂等）", async () => {
      // Mock 文章存在
      vi.mocked(prisma.post.findFirst).mockResolvedValueOnce({
        id: postId,
        published: true,
    ...
    it("setBookmark(desired=true) 应该在文章不存在时抛出错误", async () => {
      // Mock 文章不存在
      vi.mocked(prisma.post.findFirst).mockResolvedValueOnce(null)

      await expect(setBookmark(postId, userId, true)).rejects.toBeInstanceOf(
        InteractionTargetNotFoundError
      )
  ...
  describe("ensureBookmarked（幂等接口）", () => {
    const postId = "post-123"
    const userId = "user-456"

    it("多次并发调用 ensureBookmarked 应保证最终状态为已收藏", async () => {
      // Mock 文章存在
      ...
      // Mock 第一次创建成功，后续抛出 P2002
      let createCount = 0
      vi.mocked(prisma.bookmark.create).mockImplementation(async () => {
        createCount++
        if (createCount === 1) {
          return {
            id: "bookmark-1",
      ...
      const results = await Promise.all([
        ensureBookmarked(postId, userId),
        ensureBookmarked(postId, userId),
        ensureBookmarked(postId, userId),
      ])
    ...
    it("ensureBookmarked 应该在文章不存在时抛出错误", async () => {
      // Mock 文章不存在
      vi.mocked(prisma.post.findFirst).mockResolvedValueOnce(null)

      await expect(ensureBookmarked(postId, userId)).rejects.toBeInstanceOf(
        InteractionTargetNotFoundError
      )
    })
  })

  describe("ensureUnbookmarked（幂等接口）", () => {
    const postId = "post-123"
    const userId = "user-456"

    it("多次并发调用 ensureUnbookmarked 应保证最终状态为未收藏", async () => {
      // Mock deleteMany 天然幂等
      vi.mocked(prisma.bookmark.deleteMany).mockResolvedValue({ count: 1 })
      vi.mocked(prisma.bookmark.count).mockResolvedValue(0)

      // 并发调用 3 次
      const results = await Promise.all([
        ensureUnbookmarked(postId, userId),
        ensureUnbookmarked(postId, userId),
        ensureUnbookmarked(postId, userId),
      ])
      ...
      expect(prisma.bookmark.deleteMany).toHaveBeenCalledTimes(3)
    })

    it("ensureUnbookmarked 应该在收藏不存在时也返回成功（幂等）", async () => {
      // Mock deleteMany 返回 0（没有删除任何记录）
      vi.mocked(prisma.bookmark.deleteMany).mockResolvedValueOnce({ count: 0 })
      vi.mocked(prisma.bookmark.count).mockResolvedValueOnce(0)

      const result = await ensureUnbookmarked(postId, userId)
