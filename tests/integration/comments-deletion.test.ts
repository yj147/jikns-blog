...
describe("Comment Deletion API Tests", () => {
  ...
  describe("Soft Delete (with replies)", () => {
    it("should soft delete comment when it has replies", async () => {
      ...
      // 验证调用了软删除（只更新 deletedAt 字段，不修改 content）
      // 占位符替换在 formatComment 函数中进行，不在数据库层
  ...
  describe("Hard Delete (no replies)", () => {
    ...
    it("should decrement activity.commentsCount on hard delete", async () => {
      ...
      // 注意：计数更新由数据库触发器自动处理
      // 见 supabase/migrations/20251101120000_fix_comment_count_soft_delete_strategy.sql
      // DELETE 操作会触发 sync_activity_comments_count_trigger，自动 decrement commentsCount
      // 服务层代码不再手动更新计数
    })
