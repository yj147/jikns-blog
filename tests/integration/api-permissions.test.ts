...
describe("API 权限控制集成测试", () => {
  ...
  describe("权限验证工具函数测试", () => {
    it("requireAuth() 应正确验证认证状态", async () => {
      ...
      // 未认证用户
      setCurrentTestUser(null)(mockPrisma.user.findUnique as any).mockResolvedValue(null)
...
async function simulateApiHandler(request: Request, body?: any) {
  ...
  try {
    ...
    if (pathname.startsWith("/api/admin/")) {
      const { validateApiPermissions } = await import("@/lib/permissions")
      const result = await validateApiPermissions(request, "admin")
      if (!result.success) {
        return {
      ...
      const { validateApiPermissions } = await import("@/lib/permissions")
      const result = await validateApiPermissions(request, "auth")
      if (!result.success) {
        return {
          status: result.error.statusCode,
          data: result.error,
