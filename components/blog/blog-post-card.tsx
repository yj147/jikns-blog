...
import { formatRelativeTime, formatNumber, calculateReadTime } from "@/lib/utils/blog-helpers"
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="text-muted-foreground flex gap-6">
          <Button
            variant="ghost"
            size="sm"
            className={cn("group -ml-2 h-9 gap-2 rounded-full px-2", interactionStyles.comment)}
          >
            <MessageCircle className="h-5 w-5" />
            <span className="text-sm">
              {post.stats.commentsCount > 0 ? formatNumber(post.stats.commentsCount) : "评论"}
            </span>
          </Button>
          ...
          <Button
            variant="ghost"
            size="sm"
            className={cn("group h-9 gap-2 rounded-full px-2", interactionStyles.like)}
          >
            <Heart className="h-5 w-5" />
            <span className="text-sm">
              {post.stats.likesCount > 0 ? formatNumber(post.stats.likesCount) : "点赞"}
            </span>
          </Button>
          ...
          <Button
            variant="ghost"
            size="sm"
            className="hover:bg-primary/10 hover:text-primary group h-9 gap-2 rounded-full px-2 transition-colors"
          >
            <BarChart2 className="h-5 w-5" />
            <span className="text-sm">
              {post.viewCount > 0 ? formatNumber(post.viewCount) : "浏览"}
            </span>
          </Button>
        </div>
        ...
        <Button
          variant="ghost"
          size="icon"
          className={cn("text-muted-foreground h-9 w-9 rounded-full", interactionStyles.share)}
        >
